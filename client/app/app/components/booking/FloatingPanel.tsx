...
export default function FloatingPanel({ rooms }: { rooms: Room[] }) {
  ...
  const moreRoomsRequired = formContext.guestPool.adults !== 0 || formContext.guestPool.children !== 0
  ...
  return <div className={`${panelOffScreen ? "fixed top-4" : "absolute top-4"} z-20 top-0 w-full flex flex-col items-center justify-start`}>
    <div className={`absolute 2xl:w-[1100px] ${moreRoomsRequired ? "h-16" : "h-10"} transition-all duration-200 drop-shadow-sm rounded-sm bg-bg`}></div>
    <div className="flex flex-col justify-start">
      <div className={`z-10 flex justify-between items-center w-[1100px] mt-2 h-6`}>
        ...
        <button onClick={handleBookClick} className="w-[112px] cursor-pointer hover:underline">
          ...
          <div className={`${!moreRoomsRequired ? "border-b-2 border-peach " : "text-gray-warm-inactive"} mr-1 font-medium text-lg `}>
            <div>{t('Book')}</div>
          </div>
          {/* {bookingArrowSmall} */}
          {/* </div> */}
